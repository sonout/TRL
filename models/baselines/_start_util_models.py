...
def drop_path_func(x, drop_prob=0.0, training=False):
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids=None):
        ...
        if position_ids is None:
            ...
            output_pe = (
                pe[position_ids].reshape(batch_size, seq_len, self.d_model).detach()
            )
            return output_pe
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.load_trans_prob:
            self.linear_proj_tran_prob = nn.Linear(
                1, num_of_heads * num_out_features, bias=False
            )
        ...
        if self.load_trans_prob:
            self.scoring_trans_prob = nn.Parameter(
                torch.Tensor(1, num_of_heads, num_out_features)
            )
...
class GATLayerImp3(GATLayer):
    ...
    def forward(self, data):
        ...
        if self.load_trans_prob:
            ...
            scores_trans_prob = (trans_prob_proj * self.scoring_trans_prob).sum(
                dim=-1
            )  # (E, NH)
            scores_per_edge = self.leakyReLU(
                scores_source_lifted + scores_target_lifted + scores_trans_prob
            )
        else:
            scores_per_edge = self.leakyReLU(
...
class GAT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.d_model = d_model
        assert len(num_heads_per_layer) == len(
            num_features_per_layer
    ...
    def forward(self, node_features, edge_index_input, edge_prob_input, x):
        ...
        (node_fea_emb, edge_index, edge_prob) = self.gat_net(
            data
        )  # (vocab_size, num_channels[-1]), (2, E)
        batch_size, seq_len = x.shape
        node_fea_emb = node_fea_emb.expand(
            (batch_size, -1, -1)
        )  # (B, vocab_size, d_model)
        node_fea_emb = node_fea_emb.reshape(
            -1, self.d_model
        )  # (B * vocab_size, d_model)
        x = x.reshape(-1, 1).squeeze(1)  # (B * T,)
        out_node_fea_emb = node_fea_emb[x].reshape(
...
class BERTEmbedding(nn.Module):
    def __init__(
        ...
        node_fea_dim=10,
        add_gat=True,
        gat_heads_per_layer=None,
        gat_features_per_layer=None,
        gat_dropout=0.6,
        load_trans_prob=True,
        avg_last=True,
    ):
    ...
    def __init__(
        ...
        if self.add_temporal_bias:
            if self.temporal_bias_dim != 0 and self.temporal_bias_dim != -1:
                self.temporal_mat_bias_1 = nn.Linear(
                    1, self.temporal_bias_dim, bias=True
                )
                self.temporal_mat_bias_2 = nn.Linear(
                    self.temporal_bias_dim, 1, bias=True
                )
    ...
    def forward(
        ...
        if self.add_temporal_bias:
            ...
            if self.temporal_bias_dim == -1:
                batch_temporal_mat = batch_temporal_mat * self.temporal_mat_bias.expand(
                    (1, seq_len, seq_len)
                )
        ...
        if future_mask:
            mask_postion = (
                torch.triu(torch.ones((1, seq_len, seq_len)), diagonal=1)
                .bool()
                .to(self.device)
            )
            if self.add_cls:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = Mlp(
            in_features=d_model,
            hidden_features=feed_forward_hidden,
...
class BERT(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.gat_features_per_layer = self.config.get(
            "gat_features_per_layer", [16, self.d_model]
        )
        ...
        self.embedding = BERTEmbedding(
            ...
            load_trans_prob=self.load_trans_prob,
            avg_last=self.gat_avg_last,
        )

        # multi-layers transformer blocks, deep network
        enc_dpr = [
            x.item() for x in torch.linspace(0, self.drop_path, self.n_layers)
    ...
    def _shuffle_position_ids(self, x, padding_masks, position_ids):
        ...
        for bsz_id in range(batch_size):
            ...
            shuffled_pid.append(
                torch.index_select(
                    sample_pid, 0, torch.tensor(total_indexes).to(device=self.device)
                ).unsqueeze(0)
            )
        shuffled_pid = torch.cat(shuffled_pid, 0).to(
            device=self.device
    ...
    def _cutoff_embeddings(
        self, embedding_output, padding_masks, direction, cutoff_rate=0.2
    ):
        ...
        for bsz_id in range(batch_size):
            ...
            if direction == "row":
                ...
                raise ValueError(
                    f"direction should be either row or column, but got {direction}"
                )
            ...
            if direction == "random":
                sample_embedding = sample_embedding.reshape(
                    -1
                )  # (seq_length * d_model, )
            cutoff_embedding = torch.index_fill(
                sample_embedding,
                dim_index,
                torch.tensor(cutoff_indexes, dtype=torch.long).to(device=self.device),
                0.0,
            )
            if direction == "random":
                cutoff_embedding = cutoff_embedding.reshape(seq_len, d_model)
            cutoff_embeddings.append(cutoff_embedding.unsqueeze(0))
        cutoff_embeddings = torch.cat(cutoff_embeddings, 0).to(
            device=self.device
    ...
    def _shuffle_embeddings(self, embedding_output, padding_masks):
        ...
        for bsz_id in range(batch_size):
            ...
            rest_indexes = list(range(num_tokens, seq_len))
            total_indexes = indexes + rest_indexes
            shuffled_embeddings.append(
                torch.index_select(
                    sample_embedding,
                    0,
                    torch.tensor(total_indexes).to(device=self.device),
                ).unsqueeze(0)
            )
        shuffled_embeddings = torch.cat(shuffled_embeddings, 0).to(
            device=self.device
        )  # (batch_size, seq_length, d_model)
    ...
    def forward(
        ...
        embedding_output = self.embedding(
            sequence=x, position_ids=position_ids, graph_dict=graph_dict
        )  # (B, T, d_model)
        ...
        if argument_methods is not None:
            if "cutoff_row" in argument_methods:
                embedding_output = self._cutoff_embeddings(
                    ...
                    padding_masks=padding_masks,
                    direction="row",
                    cutoff_rate=self.cutoff_row_rate,
                )
            if "cutoff_column" in argument_methods:
                embedding_output = self._cutoff_embeddings(
                    ...
                    padding_masks=padding_masks,
                    direction="column",
                    cutoff_rate=self.cutoff_column_rate,
                )
            if "cutoff_random" in argument_methods:
                embedding_output = self._cutoff_embeddings(
                    ...
                    padding_masks=padding_masks,
                    direction="random",
                    cutoff_rate=self.cutoff_random_rate,
                )
        ...
        padding_masks_input = (
            padding_masks.unsqueeze(1).repeat(1, x.size(1), 1).unsqueeze(1)
        )  # (B, 1, T, T)
        ...
        return (
            ...
            all_hidden_states,
            all_self_attentions,
        )  # (B, T, d_model), list of (B, T, d_model), list of (B, head, T, T)
...
class BERTDownstream(nn.Module):
    ...
    def forward(
        ...
        token_emb, hidden_states, _ = self.bert(
            x=x,
            padding_masks=padding_masks,
            batch_temporal_mat=batch_temporal_mat,
        ...
        if self.pooling == "cls" or self.pooling == "cls_before_pooler":
            ...
            token_emb[input_mask_expanded == 0] = float(
                "-inf"
            ...
            input_mask_expanded = (
                padding_masks.unsqueeze(-1).expand(avg_emb.size()).float()
            )  # (batch_size, seq_length, d_model)
...
class BERTContrastiveLM(nn.Module):
    ...
    def forward(
        ...
        padding_masks1=None,
        padding_masks2=None,
        batch_temporal_mat1=None,
        batch_temporal_mat2=None,
        graph_dict=None,
        output_hidden_states=False,
        output_attentions=False,
    ):
        ...
        out_view1, hidden_states1, _ = self.bert(
            x=contra_view1,
            padding_masks=padding_masks1,
            batch_temporal_mat=batch_temporal_mat1,
            argument_methods=argument_methods1,
            graph_dict=graph_dict,
            output_hidden_states=output_hidden_states,
            output_attentions=output_attentions,
        )  # (B, T, d_model)
        pool_out_view1 = self.pooler(
            bert_output=out_view1,
            padding_masks=padding_masks1,
            hidden_states=hidden_states1,
        )  # (B, d_model)

        out_view2, hidden_states2, _ = self.bert(
            x=contra_view2,
            padding_masks=padding_masks2,
        ...
        pool_out_view2 = self.pooler(
            bert_output=out_view2,
            padding_masks=padding_masks2,
            hidden_states=hidden_states2,
        )  # (B, d_model)

        bert_output, _, _ = self.bert(
            x=masked_input,
            padding_masks=padding_masks,
            batch_temporal_mat=batch_temporal_mat,
            graph_dict=graph_dict,
...
class BERTContrastive(nn.Module):
    ...
    def forward(
        ...
        x = self.pooler(
            bert_output=x, padding_masks=padding_masks, hidden_states=hidden_states
        )  # (B, d_model)
        return x
